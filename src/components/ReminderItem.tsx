...
import type { ReminderResponse, ListResponse, OwnerResponse, TagResponse } from '@/types/api';
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {reminder.tags && reminder.tags.length > 0 && (
              <span className="text-xs text-apple-blue flex items-center gap-0.5">
...
const ReminderItemEditMode = memo(function ReminderItemEditMode({
  ...
  const [editCustomUnit, setEditCustomUnit] = useState(reminder.custom_recurrence_unit || 'day');
            ...
            <div className="flex flex-col gap-2" onClick={stopPropagation}>
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                <button onClick={(e) => toggleDropdown('date', e)}
                  className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-[#F2F2F7] rounded-[8px] text-xs font-medium text-gray-700 hover:bg-[#E5E5EA] transition-colors">
                ...
                <button onClick={() => setEditIsFlagged(!editIsFlagged)}
                  className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-[8px] text-xs font-medium transition-colors ${editIsFlagged ? 'bg-orange-100 text-orange-500' : 'bg-[#F2F2F7] text-gray-700 hover:bg-[#E5E5EA]'}`}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill={editIsFlagged ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" x2="4" y1="22" y2="15"/></svg>
                </button>
                {/* 重复 */}
