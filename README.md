...
- 右键或编辑里改列表：同样带动子提醒树
- 拷贝粘贴：新建整棵子提醒树（相对父子关系保留在副本内，根节点为独立提醒）；每条复制子任务；复制后可连续粘贴
- 列表较长时虚拟化滚动（分组视图与编辑中除外）

### 日历

- 日 / 周 / 月 / 年视图；按可见日期范围在前端过滤有截止日期的提醒
- 日 / 周：0–23 时间轴；双击时间轴预填时刻新建；双击全天格预填当天全天新建
- 日视图：顶部周条可点选日期；全天事项可拖到周条其它日改期
- 周视图：点日期表头进入日视图
- 月：单击进入日视图；双击空白格预填当天全天新建
- 年：点月份名进入月视图；点日期进入日视图；双击日期预填当天全天新建
...
- 系统通知：每 20 秒轮询未完成项；若设置了提前提醒，提前时刻与到期时刻各弹一次（正文含截止时间）；启动时补扫一次；去重窗口约 10 分钟；点击通知会显示窗口并打开对应提醒
