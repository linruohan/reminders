...
- ID：UUID v4 字符串
- 扩展字段：全天、旗标、优先级、重复、提前提醒、负责人、标签、URL、子任务、父子提醒（`parent_id`）
...
├── components/
│   ├── reminder/           # FormFields、内联编辑、表单选项、子任务
│   ├── calendar/           # 日/周/月/年视图与编辑卡片
│   └── ...
├── hooks/                  # useReminderData、useVirtualList、useTheme
└── utils/                  # reminderForm、reminderUpdates、dateUtils
```

后端：`src-tauri/src/commands/`、`repository/`、`database/`

## 五、表单与编辑

- **共享字段**：`ReminderFormFields`（新建弹窗 / 日历编辑）；内联编辑为 chip + 下拉，字段集相同（含负责人）
...
| 能力 | 说明 |
|------|------|
| 视图 | 日 / 周 / 月 / 年；按可见日期范围在前端过滤 |
| 新建 | 双击时间轴预填时刻；全天行预填全天；月视图双击空白格预填当天全天；日视图周条可点选日期，全天可拖到其它日 |
| 改期 | 有时事件拖拽（15 分钟吸附）；全天/月视图拖到其它日 |

## 七、仍待（可选产品能力）

- 可选 lint/format 脚本
- 后端 FTS / 按日期范围拉取（当前数据量走全量加载 + 前端过滤）
...
- 列表只读勾选；内联编辑 / 详情可增删改
- 命令：`create_subtask` / `update_subtask` / `delete_subtask`（无独立 get）
- 父子提醒：`parent_id`；删除父任务级联删除子提醒；完成重复项时下一实例继承 `parent_id` 并复制子任务（未完成）
- 剪切：更新 `list_id`（含子提醒），不删原记录；拷贝：递归复制子提醒树并复制子任务，粘贴根为独立提醒
